...
| `am usage [day\|week\|month\|all]` | tokens used through the proxy, total per day (table) — default: week |
| `am usage -D\|--detail [-d\|--date YYYY-MM-DD] [-p\|--project NAME]` | breakdown by account, model, project, and session (running total per session, resets on `/clear`) — `-d` pins one day, `-p` filters one project, combinable |
